...
import { getSalas, actualizarOcupacion } from "../services/mockService"
...
export default function HomeScreen() {
  ...
  const [filters, setFilters] = useState({
    ...
    silence: "all", // 'alto', 'medio', 'bajo', 'all'
    size: "all", // 'pequeña', 'mediana', 'grande', 'all'
  ...
  // Función para cargar las salas inicialmente
  const loadRooms = () => {
    try {
      const data = getSalas()
      setRooms(data)
  ...
  // Función para actualizar la ocupación de las salas
  const updateRoomOccupancy = () => {
    try {
      const updatedRooms = actualizarOcupacion(rooms)
  ...
  const getMarkerColor = (ocupacion, capacidad) => {
      ...
      {loading ? (
        ...
        <>
          ...
          <View style={styles.autoUpdateContainer}>
            <Ionicons name="time-outline" size={16} color="#666" style={styles.autoUpdateIcon} />
            <Text style={styles.autoUpdateText}>Actualización automática cada 10s</Text>
          </View>
        </>
      )}
